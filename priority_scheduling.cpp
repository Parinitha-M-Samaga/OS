...
class Process
{
	public:
		string name;
		int arrival, burst, prival, finish, turn_around, waiting, exe, turn;
	Process()
	{
		turn_around=0;
		waiting=0;
...
void priority(vector <Process> &process)
{
	int min=9999, pr=9999, ind, ct=0;
	for(int i=0;i<process.size();++i)
		if(process[i].arrival<min and process[i].prival<pr)
		{
	...
	while(nind<=process.size())
	{
		...
		if(next.size()==0)
		{
			min=9999; pr=9999;
			for(int i=0;i<process.size();++i)
				if(process[i].exe==0 and process[i].arrival<=min and process[i].prival<pr)
				{
					min=process[i].arrival;
					pr=process[i].prival;
					ind=i;
